...
public class BarrilControlador {
    ...
    public String listarBarriles(
        ...
        // Formatear las fechas antes de agregarlas al modelo
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
    
        barrilesPage.getContent().forEach(barril -> {
            if (barril.getFechaCarga() != null) {
                barril.setFormattedFechaCarga(barril.getFechaCarga().format(formatter));
            }
            if (barril.getFechaVencimiento() != null) {
                barril.setFormattedFechaVencimiento(barril.getFechaVencimiento().format(formatter));
            }
        });
...
public String actualizarBarril(@PathVariable Long id, @ModelAttribute("barril") Barril barril) {
    ...
    barrilExistente.setFechaCarga(barril.getFechaCarga());
    barrilExistente.setFechaVencimiento(barril.getFechaVencimiento());
    ...
    public String obtenerBarrilPorId(@PathVariable Long id, Model modelo) {
        ...
        // Agregar una variable indicando si lote está asignado o no
        modelo.addAttribute("loteAsignado", barril.getLote() != null ? barril.getLote().getId() : "Sin lote asignado");
        modelo.addAttribute("barril", barril);
        return "show_barril";
    }
